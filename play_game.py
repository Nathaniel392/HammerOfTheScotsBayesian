...
def set_up_combat_dict(current_board, battle_region):
    ...
    for block in current_board.regions[battle_regionID].blocks_present:
        for path in current_board.regions[battle_regionID].enterers['SCOTLAND']:
            if block in current_board.regions[battle_regionID].enterers['SCOTLAND'][path]:
                is_defender = False
        for path in current_board.regions[battle_regionID].enterers['ENGLAND']:
            if block in current_board.regions[battle_regionID].enterers['ENGLAND'][path]:
                is_defender = False
        if is_defender:
    ...
    for i,path in enumerate(current_board.regions[battle_regionID].enterers[attacking_allegiance]):
        if i == 0:
            for block,order in current_board.regions[battle_regionID].enterers[attacking_allegiance][path]:
                current_board.regions[battle_regionID].combat_dict['Attacking'].append(block)
                if order < first_attacking_order:
                    first_attacking_order = order
        else:
            for block,order in current_board.regions[battle_regionID].enterers[attacking_allegiance][path]:
        ...
        while play_turn:
            ...
            while len(contested_regions) > 0:
                ...
                if who_goes_first == False:
                    ...
                    set_up_combat_dict(current_board, battle_region)
